...
class OrdemService implements IOrdemService {
  ...
  public extrairOrdensDeArquivo(file: any): Promise<void> {

    return new Promise<void>((resultPromisse, RejectPromisse) => {
      this._OrdemRepository.deletetarTotas(this._authService.recuperaUsuarioLogado().uid)
      .then(result => {
